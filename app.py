...
if uploaded_file:
    try:
        ...
        # Gráfico de evolução
        # -----------------------------
        st.subheader("📊 Evolução das colunas B, C e Diferença")
        st.line_chart(df[['B','C','Diferença']])

        # -----------------------------
        # Download CSV com resultados
        # -----------------------------
        df_download = df.copy()
        df_download['B'] = df_download['B'].apply(formatar_brl)
        df_download['C'] = df_download['C'].apply(formatar_brl)
        df_download['Diferença'] = df_download['Diferença'].apply(formatar_brl)
        df_download['Cashback Percentual'] = f"{percentual*100:.0f}%"
        df_download['Resultado Final'] = formatar_brl(resultado_final)

        csv_buffer = df_download.to_csv(index=False).encode('utf-8')
        st.download_button(
            label="⬇️ Baixar CSV com resultados",
            data=csv_buffer,
            file_name="resultado_cashback.csv",
            mime="text/csv"
        )
